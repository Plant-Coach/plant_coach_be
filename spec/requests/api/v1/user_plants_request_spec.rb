...
RSpec.describe 'User Plants API Endpoint' do
  describe 'POST user plants' do
    it 'creates new plant that the user will be planting' do
      ...
      post '/api/v1/user_plants', params: { user_id: user.id, plant_id: plant.id}, headers: { Authorization: "Bearer #{user_response[:jwt]}" }
    ...
    it 'will return a json error message if there was a problem' do
      ...
      post '/api/v1/user_plants', params: { user_id: nil, plant_id: plant.id}, headers: { Authorization: "Bearer #{user_response[:jwt]}" }
      result = JSON.parse(response.body, symbolize_names: true)
  ...
  describe 'DELETE /user_plants' do
    it 'removes the plant from the users list of plants' do
      ...
      delete "/api/v1/user_plants/#{user_plant.id}", headers: { Authorization: "Bearer #{user_response[:jwt]}" } # Would like to refactor this to use params but it does not like them for some reason.
      result = JSON.parse(response.body, symbolize_names: true)
